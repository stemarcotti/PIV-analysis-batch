...
for file_list = 1:n_files
    ...
    centroid_to_largest = [smooth(centroid_to_largest(:,1),5) smooth(centroid_to_largest(:,2),5)];
    
    nt = length(coord_largest_ext);
    
    % calculate cosine between vectors
    costheta_out = ones(nt,1);
    for k = 1:nt-1
        
        A = centroid_to_largest(1:nt-k, :);
        B = centroid_to_largest(1+k:end,:);
