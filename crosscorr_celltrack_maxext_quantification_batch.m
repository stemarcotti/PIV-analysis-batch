...
%         (coord_primary_sink(:,2)-track(:,2))];
    ...
    [corr1, corr2, lagstore] = crosscorr_quantification(track_diff, centroid_to_largest);
    crosscorr_cell_maxext_all(file_list).crosscorr1 = corr1;
    crosscorr_cell_maxext_all(file_list).crosscorr2 = corr2;
    crosscorr_cell_maxext_all(file_list).lag = lagstore;
    
    % clear output variables
    clear parameters track
    clear centroid_to_largest
...
for ii = 1:file_list
    len(ii,1) = length(crosscorr_cell_maxext_all(ii).crosscorr1);
end
max_len = max(len);
...
for k = 1:file_list
    temp1 = crosscorr_cell_maxext_all(k).crosscorr1;
    temp2 = crosscorr_cell_maxext_all(k).crosscorr2;
    temp3 = crosscorr_cell_maxext_all(k).lag;
...
crosscorr_cell_maxext_weighted = [weighted_avg2; weighted_avg];
...
save(fullfile(parent_d, ...
    'RPE_crosscorr_cell_maxext_weighted_avg.mat'), ...
    'lags', 'crosscorr_cell_maxext_weighted');