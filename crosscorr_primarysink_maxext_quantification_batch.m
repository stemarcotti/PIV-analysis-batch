...
%     track_diff = [diff(track(:,1)) diff(track(:,2))];
    ...
    [corr1, corr2, lagstore] = crosscorr_quantification(centroid_to_primary, centroid_to_largest);
    crosscorr_primarysink_maxext_all(file_list).crosscorr1 = corr1;
    crosscorr_primarysink_maxext_all(file_list).crosscorr2 = corr2;
    crosscorr_primarysink_maxext_all(file_list).lag = lagstore;
    
    % clear output variables
    clear parameters track
    clear centroid_to_largest centroid_to_primary
...
for ii = 1:file_list
    len(ii,1) = length(crosscorr_primarysink_maxext_all(ii).crosscorr1);
end
max_len = max(len);
...
for k = 1:file_list
    temp1 = crosscorr_primarysink_maxext_all(k).crosscorr1;
    temp2 = crosscorr_primarysink_maxext_all(k).crosscorr2;
    temp3 = crosscorr_primarysink_maxext_all(k).lag;
...
crosscorr_primarysink_maxext_weighted = [weighted_avg2; weighted_avg];
...
save(fullfile(parent_d, ...
    'RPE_crosscorr_primarysink_maxext_weighted_avg.mat'), ...
    'lags', 'crosscorr_primarysink_maxext_weighted');